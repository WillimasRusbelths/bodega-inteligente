...
describe("inventory BI dashboard acquisition [T024]", () => {
  it("keeps fulfilled BI resources available when another existing read fails", async () => {
    const api = {
      ...
      expirationRisk: () => Promise.reject(new Error("offline")),
      movementSummary: () => Promise.resolve([]),
      alertsSummary: () => Promise.resolve([]),
    ...
    expect(controller.state.resources?.expirationRisk).toEqual({
      status: "ERROR",
      message: "No se pudo cargar el resumen BI de inventario.",
    });
